...
async function run() {
    ...
    const injectPayload = async () => {
        const themeData = await hubPage.evaluate(() => {
            const themes = JSON.parse(localStorage.getItem('jdroidx-matrix-themes') || '[]');
            const elements = JSON.parse(localStorage.getItem('jdroidx-matrix-elements') || '[]');
            const activeThemeIdx = themes.findIndex(t => t.active);
            const idx = activeThemeIdx !== -1 ? activeThemeIdx : 0;
            
            const data = {};
            elements.forEach(el => {
                const varNames = el.mappings.length > 0 ? el.mappings : [el.name.toLowerCase().replace(/ \/ /g, '-').replace(/ & /g, '-').replace(/ /g, '-')];
                varNames.forEach(v => {
                    data[v] = el.colors[idx];
                });
            });
            return data;
        });
        ...
        await targetPage.evaluate((data) => {
            ...
            css += '}';
            const styleId = 'jdroidx-automation-bridge';
            ...
            console.log("%c⚡ JDROID-X AUTOMATION: Atmosphere Pulsed", "color:#FF9933; font-weight:bold;");
        }, themeData);
    };
    ...
    setInterval(async () => {
        try {
            const currentData = await hubPage.evaluate(() => {
                return localStorage.getItem('jdroidx-matrix-elements') + localStorage.getItem('jdroidx-matrix-themes');
            });
