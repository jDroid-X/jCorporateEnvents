...
const ThemeManager = {
    ...
    setTheme(themeKey, themeName, customColors = null) {
        ...
        const properties = ['--bg', '--side-bg', '--header-bg', '--footer-bg', '--btn-bg', '--text', '--accent', '--accent-soft', '--highlight-gold', '--border', '--card-bg', '--scroll-thumb', '--status-ok'];
        ...
        if (customColors && customColors.length > 0) {
            ...
            if (customColors[0]) root.style.setProperty('--bg', customColors[0]);
            if (customColors[1]) root.style.setProperty('--side-bg', customColors[1]);
            if (customColors[2]) root.style.setProperty('--header-bg', customColors[2]);
            if (customColors[2]) root.style.setProperty('--footer-bg', customColors[2]);
            if (customColors[3]) root.style.setProperty('--btn-bg', customColors[3]);
            if (customColors[4]) root.style.setProperty('--accent', customColors[4]);
            if (customColors[5]) root.style.setProperty('--accent-soft', customColors[5]);
            if (customColors[6] || customColors[5]) root.style.setProperty('--text', customColors[6] || customColors[5]);
            if (customColors[7] || customColors[6]) root.style.setProperty('--highlight-gold', customColors[7] || customColors[6]);
        ...
        const menu = document.getElementById('theme-popup-menu');
        if (menu) menu.style.display = 'none';

        // Dispatch custom event for app logic
        document.dispatchEvent(new CustomEvent('themeChanged', { detail: { key: themeKey, name: themeName } }));
    },
    ...
    applyActiveTheme() {
        try {
            ...
            if (savedName && savedName !== 'Default Glass') {
                const savedThemes = JSON.parse(localStorage.getItem(this.keys.THEMES) || '[]');
                const savedElements = JSON.parse(localStorage.getItem(this.keys.ELEMENTS) || '[]');
            ...
            this.setTheme(savedKey, savedName || 'Default Glass');
        } catch (e) {
            this.setTheme('default', 'Default Glass');
        }
    },
    ...
    initSync() {
        // Record caller for designer context
