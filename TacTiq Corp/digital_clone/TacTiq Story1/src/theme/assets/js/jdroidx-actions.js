...
JdroidX.Actions = {
    ...
    remapElement: function (i) {
        const next = prompt("Target CSS Variable:", JdroidX.state.elements[i].mapping);
        if (next) { JdroidX.state.elements[i].mapping = next.replace(/^--/, ''); this.refresh(); }
    },

    toggleMapping: function (i) {
        alert("Layer Synchronized: This mapping is active in the master matrix.");
    },
