...
const ThemeManager = {
    ...
    setTheme(themeKey, themeName, customColors = null) {
        ...
        ['--bg', '--side-bg', '--header-bg', '--footer-bg', '--btn-bg', '--text', '--accent', '--accent-soft', '--highlight-gold', '--border', '--card-bg', '--scroll-thumb', '--scroll-track', '--font-p', '--font-h', '--status-ok'].forEach(p => body.style.removeProperty(p));
        ...
        if (customColors && customColors.length >= 7) {
            console.log(`ThemeManager: Applying Custom Theme [${themeName}]`, customColors);
            ...
            body.style.setProperty('--footer-bg', customColors[2]);
            ...
            if (customColors[11]) body.style.setProperty('--scroll-track', customColors[11]);
            if (customColors[12]) body.style.setProperty('--font-p', customColors[12]);
            if (customColors[13]) body.style.setProperty('--font-h', customColors[13]);
            if (customColors[14]) body.style.setProperty('--status-ok', customColors[14]);
        } else {
            console.log(`ThemeManager: Setting Standard Theme [${themeKey}]`);
        }
