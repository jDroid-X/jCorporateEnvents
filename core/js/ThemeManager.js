...
const ThemeManager = {
    ...
    setTheme(themeKey, themeName, customColors = null) {
        ...
        ['--bg', '--side-bg', '--header-bg', '--footer-bg', '--btn-bg', '--text', '--accent', '--accent-soft', '--highlight-gold', '--border', '--card-bg', '--scroll-thumb', '--status-ok'].forEach(p => body.style.removeProperty(p));
        ...
        if (customColors && customColors.length >= 7) {
            ...
            body.style.setProperty('--footer-bg', customColors[2]); // Sync footer with header color
